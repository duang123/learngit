...
*/
	struct pollfd conn_arr[1024];
	int i,nfds;
	int maxi=0;
	for(i=0;i<1024;++i)conn_arr[i].fd=-1;
	conn_arr[0].fd=listenfd;
	conn_arr[0].events=POLLIN;
	while(1){
		nfds=poll(conn_arr,maxi+1,-1);
		if(nfds==-1){
			if(errno==EINTR)continue;
				ERR_EXIT("poll");
		}else if(nfds==0){continue;}//超时继续
		if(conn_arr[0].revents & POLLIN){//产生可读事件
